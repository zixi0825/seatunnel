...
import io.github.interestinglab.waterdrop.spark.structuredstream.StructuredStreamingSource
import io.github.interestinglab.waterdrop.spark.utils.SparkStructTypeUtil
...
class KafkaStream extends StructuredStreamingSource {
  ...
  override def prepare(env: SparkEnvironment): Unit = {
    ...
    //offset异步回写
    env.getSparkSession.streams.addListener(new StreamingQueryListener() {
  ...
  override def getData(env: SparkEnvironment): Dataset[Row] = {
    ...
    var dataFrame = env.getSparkSession.readStream
      .format("kafka")
